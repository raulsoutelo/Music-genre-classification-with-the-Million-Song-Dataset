...
#WE REMOVE SOME OF THE WEIGHTS 
hidden1_weights_L1 = load_from_file('hidden1_weights_L1')
hidden2_weights_L1 = load_from_file('hidden2_weights_L1')
hidden1_weights_mask = tf.constant(0.0, shape=[25, 50])
threshold = 0.001
for i in range(0,25):
	for j in range (0, 50):
		if hidden1_weights_L1[i, j] > threshold or hidden1_weights_L1[i, j] < -threshold:
			indices = [[i, j]]  
			values = [1.0] 
			shape = [25, 50] 
			delta = tf.SparseTensor(indices, values, shape)
			hidden1_weights_mask = hidden1_weights_mask + tf.sparse_tensor_to_dense(delta)

hidden2_weights_mask = tf.constant(0.0, shape=[50, 50])
for i in range(0,50):
	for j in range(0, 50):
		if hidden2_weights_L1[i, j] > threshold or hidden2_weights_L1[i, j] < -threshold:
			indices = [[i, j]]  
			values = [1.0] 
			shape = [50, 50] 
			delta = tf.SparseTensor(indices, values, shape)
			hidden2_weights_mask = hidden2_weights_mask + tf.sparse_tensor_to_dense(delta)
...
def not_fully_connected_layer(inputs, segment_count, segment_dim, num_kernels, mask, nonlinearity=tf.nn.relu):
    weights = tf.Variable(
        tf.truncated_normal(
    ...
    weightsSparse = tf.multiply(weights, mask)
    output = tf.einsum('ijk,kl->ijl', inputs_1, weightsSparse) + biases
...
with tf.name_scope('layer-1'):
    hidden_1, hidden1_weights = not_fully_connected_layer(inputs, 120, 25, kernels_1, hidden1_weights_mask)
with tf.name_scope('layer-2'):
    hidden_2, hidden2_weights = not_fully_connected_layer(hidden_1, 120, kernels_1, kernels_2, hidden2_weights_mask)
...
save_to_file('err_val_L1andL2_threshold_0_001',  err_val)             
save_to_file('acc_val_L1andL2_threshold_0_001',  acc_val)
