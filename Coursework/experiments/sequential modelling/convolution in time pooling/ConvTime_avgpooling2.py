...
def conv_layer_maxpooling(inputs, image_height, image_width, in_channels, out_channels, kernel_height, kernel_width, nonlinearity=tf.nn.relu):
    ...
    output = tf.nn.avg_pool(output_no_pooling, ksize, strides2, padding)
...
with tf.name_scope('fc-layer-15'):
    hidden_1, hidden1_weights = conv_layer_maxpooling(inputs, 120, 25, 1, kernels, kernel_height, 25)
with tf.name_scope('fc-layer-16'):
    hidden_2, hidden2_weights = conv_layer_maxpooling(hidden_1,  int(np.ceil(    (120-(kernel_height -1))/2   )), kernels, 1, kernels, kernel_height, kernels)
...
save_to_file('err_val_convtime_avgpooling',  err_val)         
save_to_file('acc_val_convtime_avgpooling',  acc_val)
